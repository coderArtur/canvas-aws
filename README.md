...
- **Alterar o Modo Silencioso**: Por padrão, o robô trabalha escondido. Se você quiser *ver* a janela do navegador abrindo e o robô clicando nas coisas (modo fantasma), basta abrir o arquivo `config.py` e trocar `OCULTAR_TELA = False` para `OCULTAR_TELA = True`. (Isso é muito útil se você quiser ver a mágica acontecendo).
